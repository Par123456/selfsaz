...
import os
#================= Config =================#
owner = 6508600903 # Owner ID
api_id = 29042268 # API ID
api_hash = "54a7b377dd4a04a58108639febe2f443" # API HASH
bot_token = "8239455701:AAG3Bx6xEn42e3fggTWhcRf66-CDPQCiOZs" # Bot Token
#==========================================#
...
def answer(client, inline_query):
     my_users = load_admins()
     if inline_query.query == "panel" and inline_query.from_user.id in my_users:
     ...
     if inline_query.query == "coinprice" and inline_query.from_user.id in my_users:
...
async def call(app, call):
     ...
     if call.data != "outside":
          if call.from_user.id in my_users and int(call.from_user.id) == int(call.data.split("-")[1]):
...
@app.on_message(filters.private&owners&filters.command("restart"), group=1)
...
async def updates(app, m:Message):
 ...
 if text == "Add User":
   ...
   if int(answer.text) in my_users:
     ...
     s = ""
     if my_users:
          for index, user in enumerate(my_users, start=1):
               s += f"֍ {index} -> `{user}`\n"
   ...
   if int(answer.text) in my_users:
       ...
       await app.send_message(m.chat.id, f"ابتدا کاربر مورد نظر را به لیست User اضافه کنید❗️")
       
 elif text == "Delete Owner":
   answer = await app.ask(m.chat.id, '**Send Me User ID**:')
   if int(answer.text) in my_users:
       my_owners.remove(int(answer.text))
     ...
     s = ""
     if my_owners:
          for index, user in enumerate(my_owners, start=1):
               s += f"֍ {index} -> `{user}`\n"
