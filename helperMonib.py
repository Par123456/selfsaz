...
bot_token = "8239455701:AAG3Bx6xEn42e3fggTWhcRf66-CDPQCiOZs" # توکن ربات هلپر 
DBName = "a1176921_self1" # نام دیتابیس هلپر 
DBUser = "a1176921_self1" # یوزر دیتابیس هلپر 
DBPass = "19Nhexu0" # پسورد دیتابیس هلپر
#==========================================#
...
async def call(app, call):
     ...
     if call.data != "outside":
          try:
               ...
               action = parts[0]
               # اگر فرمت call.data درست باشد، آیدی کاربر را استخراج می‌کنیم
               # در غیر این صورت، IndexError رخ می‌دهد که توسط except مدیریت می‌شود
               callback_user_id = int(parts[1])

               if AdminUser is not None and int(call.from_user.id) == callback_user_id:
...
✨ Everything is ready for you! ✨**""", reply_markup=mark2)
               ...
               # این بلاک زمانی اجرا می‌شود که call.data فرمت مورد انتظار را نداشته باشد
               # (مثلاً خط تیره ندارد یا بخش دوم آن عدد نیست)
               # در این حالت، به سادگی هیچ کاری انجام نمی‌دهیم یا یک پیام خطا نمایش می‌دهیم
               print(f"Callback data با فرمت نامعتبر دریافت شد: {call.data}")
               await call.answer("خطا در پردازش دکمه!", show_alert=True)
     else:
